...
class BotManager {
  ...
  async _downloadFile(fileId, downloadFolder, desiredName, label, entryType) {
    ...
    const entry = { type: entryType, path: targetPath, fileName: desiredName, fileSize: stat.size, date: (/* @__PURE__ */ new Date()).toISOString() };
  ...
  _emitDownload(entry) {
    if (this.onDownload) this.onDownload(entry);
  }
  _emitError(error) {
...
function updateTrayMenu() {
  ...
  tray.setContextMenu(Menu.buildFromTemplate([
    ...
    { type: "separator" },
    {
      ...
      click: () => {
        botRunning ? handleStopBot() : mainWindow && mainWindow.webContents.send("bot:request-start");
      }
    },
    { type: "separator" },
    { label: "Quit", click: () => {
...
function createWindow() {
  ...
  mainWindow.on("close", (event) => {
    ...
    if (config.minimize_to_tray && !app.isQuitting) {
...
function handleStartBot(token, allowedUserId, downloadFolder) {
  if (!token) {
    mainWindow && mainWindow.webContents.send("bot:error", "No token provided");
    return;
  }
  if (!allowedUserId) {
    mainWindow && mainWindow.webContents.send("bot:error", "No user ID provided");
    return;
  }
  botManager.onLog = (text) => {
    mainWindow && mainWindow.webContents.send("bot:log", text);
  };
  botManager.onStatus = (status) => {
    mainWindow && mainWindow.webContents.send("bot:status", status);
    updateTrayMenu();
  };
  ...
  botManager.onError = (err) => {
    mainWindow && mainWindow.webContents.send("bot:error", err);
  };
...
function handleSelectFolder() {
  return dialog.showOpenDialog(mainWindow, { properties: ["openDirectory"], title: "Select Download Folder" }).then((r) => r.canceled ? null : r.filePaths[0]);
}
...
app.whenReady().then(() => {
  ...
  ipcMain.handle("save-config", (_, config2) => {
    saveConfig(config2);
    return true;
  });
  ...
  ipcMain.handle("delete-file", (_, p) => {
    try {
  ...
  if (config.auto_start && config.token && config.allowed_user_id) {
    setTimeout(() => handleStartBot(config.token, config.allowed_user_id, config.download_folder), 1e3);
  }
});
